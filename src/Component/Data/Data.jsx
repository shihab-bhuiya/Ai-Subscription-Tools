...
const Data = ({data, setSelectData, selectData}) => {
  ...
  const alreadySelected = selectData.some(item => item.id === data.id);
    ...
    <div className='m-5'>
      <div className="max-w-sm mx-auto bg-gray-100 rounded-2xl p-5 shadow-md relative">
        ...
        <ul className="mb-6 space-y-2 text-gray-600">
          <li className="flex items-center gap-2"><span className="text-green-500">✔</span> {data.feature1}</li>
          <li className="flex items-center gap-2"><span className="text-green-500">✔</span> {data.feature2}</li>
          <li className="flex items-center gap-2"><span className="text-green-500">✔</span> {data.feature3}</li>
        </ul>

        {/* Button */}
        <button
          className="w-full py-3 rounded-full btn btn-primary font-semibold bg-gradient-r from-purple-600 to-pink-50 hover:opacity-90 transition"
          onClick={handler}
          disabled={isSelected || alreadySelected}
        >
          {isSelected || alreadySelected ? " Added to Card" : "Buy Now"}
        </button>
      </div>
    </div>
  );
